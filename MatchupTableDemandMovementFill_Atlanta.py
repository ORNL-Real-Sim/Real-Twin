...
if __name__ == '__main__':
    ...
    project_folder_path = "datasets/Atlanta"
    ...
    def is_turn_label_consistent(row):
        turn_label = row['Turn_Label']
        if pd.isna(turn_label):
            return False

        if isinstance(turn_label, (list, tuple, set, np.ndarray)):
            labels = [str(label) for label in turn_label if pd.notna(label)]
        else:
            labels = [str(turn_label)]

        turn_synchro = str(row['Turn_Synchro'])
        return any(label in turn_synchro for label in labels)

    MatchupTable_UserInput['Synchro_GS_Consist_Verify'] = np.where(
        MatchupTable_UserInput.apply(is_turn_label_consistent, axis=1) | (~MatchupTable_UserInput['Turn_Synchro'].notnull()),
        0, 1)
