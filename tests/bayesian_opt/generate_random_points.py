...
def generateRandom(x_mins, x_maxs, num_points):
    """
    ...
    """
    dims = len(x_mins)
    x = np.zeros((num_points, dims))
    
    for i in range(dims):
        x[:, i] = np.random.uniform(x_mins[i], x_maxs[i], num_points)
...
def get_furthest_points(points, k):
    ...
    if n<=60000: # For very large datasets, use a more efficient method
        ...
        centers, indices = kmeans_plusplus(points, n_clusters=k, random_state=0)
...
def main():
    ...
    for i in range(len(param_ranges)):
        ...
        mapped_points[:, i] = selected_points[:, i] * (max_val - min_val) + min_val
