...
if __name__ == '__main__':
    ...
    project_folder_path = "datasets/Nashville_Full"
    # project_folder_path = "datasets/Roosevelt"
    ...
    # special case, delete D4435 NBR approach
    df_approach_config = df_approach_config[~((df_approach_config['JunctionID_OpenDrive'] == 'D4435') &
                                              (df_approach_config['FromRoadID_OpenDrive'] == '133450'))]

    # double check if there is still approach name overlap
    df_approach_config["approach_name_rank"] = df_approach_config.groupby(
        by=['JunctionID_OpenDrive', 'approach_direction_bound']).cumcount()

    # update the approach label for those right turn channelization junctions
    right_channel_file = rf'{project_folder_path}/RightChannelization_ID.xlsx'
    right_channel_df = pd.read_excel(right_channel_file)
    # remove the rows with all Nan values across all columns
    right_channel_df = right_channel_df.dropna(how='all')
    # forward fill Major Junction ID
    right_channel_df['Major Junction ID'] = right_channel_df['Major Junction ID'].ffill()
    right_channel_df[['Major Junction ID', 'From (Junction ID)']] = right_channel_df[['Major Junction ID', 'From (Junction ID)']].astype(str)

    RT_junction_list = right_channel_df['From (Junction ID)'].unique().tolist()

    df_approach_config_RT = df_approach_config[df_approach_config['JunctionID_OpenDrive'].isin(RT_junction_list)].reset_index(drop=True)

    # get major junction information
    df_approach_config_RT = df_approach_config_RT.merge(right_channel_df[['Major Junction ID', 'From (Junction ID)']],
                                                        left_on='JunctionID_OpenDrive', right_on='From (Junction ID)')
    df_approach_config_RT = df_approach_config_RT.merge(df_approach_config[['JunctionID_OpenDrive', 'Numbering', 'approach_direction_bound']],
                                                        left_on='Major Junction ID', right_on='JunctionID_OpenDrive',
                                                        suffixes=('', '_Major'), how='left')
    # update the approach_direction_bound based on the major junction and based on the closest bearing
    df_approach_config_RT['bearing_diff'] = abs(df_approach_config_RT['Numbering'] - df_approach_config_RT['Numbering_Major'])
    df_approach_config_RT = df_approach_config_RT.sort_values(by=['JunctionID_OpenDrive', 'bearing_diff']).reset_index(drop=True)
    df_approach_config_RT = df_approach_config_RT.groupby('JunctionID_OpenDrive', group_keys=False).first().reset_index()

    # update the df_approach_config with RT information based on the approach_direction_bound_Major
    df_approach_config = df_approach_config.merge(
        df_approach_config_RT[['JunctionID_OpenDrive', 'Numbering', 'approach_direction_bound_Major']],
        on=['JunctionID_OpenDrive', 'Numbering'],
        how='left'
    )

    df_approach_config['approach_direction_bound'] = np.where(
        df_approach_config['approach_direction_bound_Major'].notnull(),
        df_approach_config['approach_direction_bound_Major'],
        df_approach_config['approach_direction_bound']
    )

    # drop the temporary columns
    df_approach_config = df_approach_config.drop(columns='approach_direction_bound_Major')
    ...
    # verify that the RT channelized movements are correctly labeled as R and T
    # MatchupTable_UserInput_RT = MatchupTable_UserInput[MatchupTable_UserInput['JunctionID_OpenDrive'].isin(RT_junction_list) 
    #                                                    & MatchupTable_UserInput['JunctionID_OpenDrive'].isin(junction_ids_in_traffic)]
    MatchupTable_UserInput_RT = MatchupTable_UserInput[MatchupTable_UserInput['JunctionID_OpenDrive'].isin(RT_junction_list)]
    MatchupTable_UserInput_RT['Turn_Rank'] = MatchupTable_UserInput_RT.groupby(
        by=['JunctionID_OpenDrive', 'approach_direction_bound']).cumcount()
    # for those with Turn_Rank == 1, they should be labeled as 'T' and those with Turn_Rank == 0, they should be labeled as 'R'
    MatchupTable_UserInput_RT['Turn_Label'] = np.where(MatchupTable_UserInput_RT['Turn_Rank'] == 1, 'T', 'R')

    # join back to the main table
    MatchupTable_UserInput = MatchupTable_UserInput.merge(
        MatchupTable_UserInput_RT[['JunctionID_OpenDrive', 'FromRoadID_OpenDrive', 'ToRoadID_OpenDrive', 'Turn_Label']],
        on=['JunctionID_OpenDrive', 'FromRoadID_OpenDrive', 'ToRoadID_OpenDrive'],
        how='left',
        suffixes=('', '_updated')
    )
    MatchupTable_UserInput['Turn_Label'] = np.where(
        MatchupTable_UserInput['Turn_Label_updated'].notnull(),
        MatchupTable_UserInput['Turn_Label_updated'],
        MatchupTable_UserInput['Turn_Label']
    )
    MatchupTable_UserInput = MatchupTable_UserInput.drop(columns=['Turn_Label_updated'])
    ...
    signal_mapping = pd.read_csv(f"{project_folder_path}/sumo_network2signal_db_mapping.csv", dtype=str)
    ...
    # Add mannual fixs for Turn_GridSmart
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D4448') 
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '124690') 
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '119898'), 'Turn_GridSmart'] = 'EBU'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D4448')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '124690')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '119898'), 'Turn_Label'] = 'U'

    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D4435') 
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == 'B65')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '123918'), 'Turn_GridSmart'] = 'EBT'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D4435')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == 'B65')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '123918'), 'Turn_Label'] = 'T'

    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D1110')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == 'M7_net2')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == 'M8'), 'Turn'] = 'Uturn'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D1110')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == 'M7_net2')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == 'M8'), 'Turn_GridSmart'] = 'NBU'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D1110')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == 'M7_net2')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == 'M8'), 'Turn_Label'] = 'U'

    # delete those movement that do not have GS data
    traffic_has_rt = 'True'
    # traffic_has_rt = 'False'

    if traffic_has_rt == 'True':
        ...
        MatchupTable_UserInput['Turn_GridSmart'] = np.where((MatchupTable_UserInput['JunctionID_OpenDrive'].isin(junction_ids_in_traffic)) |
                                                            (MatchupTable_UserInput['JunctionID_OpenDrive'].isin(RT_junction_list)), MatchupTable_UserInput['Turn_GridSmart'], np.nan)
    ...
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == '8728')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '137932')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '128995'), 'Turn_Synchro'] = 'NWT'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D3028')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '114944')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '123952'), 'Turn_Synchro'] = 'WBT'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'D3827')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '129613')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '129602'), 'Turn_Synchro'] = 'WBT'
    MatchupTable_UserInput.loc[(MatchupTable_UserInput['JunctionID_OpenDrive'] == 'clusterJ287_J289')
                                & (MatchupTable_UserInput['FromRoadID_OpenDrive'] == '116128.5')
                                & (MatchupTable_UserInput['ToRoadID_OpenDrive'] == '116135'), 'Turn_Synchro'] = 'SBT'
    ...
    MatchupTable_UserInput.drop(columns=['Bearing_int', 'approach_direction_bound', 'approach_direction_bound_updated',
                                         'Turn_Label', 'sumo_id', 'signal_id', 'synchro_approach_name_list',
